...
import ora from "ora";
import { BASE_URL, displayInstructions, getProjectJson, getTemplateUrl, packageNameMap, } from "../utils/constant.js";
const requiredNodeVersion = 20;
const currentNodeVersion = process.versions.node.split(".")[0];
if (Number.parseInt(currentNodeVersion, 10) < requiredNodeVersion) {
    throw new Error(`Error: Node.js version ${requiredNodeVersion} or higher is required. You are using Node.js ${process.versions.node}.`);
}
export default class Create extends Command {
    static args = {};
    static description = "Create a new Celo Project";
        ...
        try {
            if (template === "") {
                ...
                for (const pkg of packages) {
                    ...
                    const packagePath = path.join(projectDir, "packages", pkg, "package.json");
                    ...
                    await fs.writeFile(packagePath, JSON.stringify(projectPackage, null, 2), "utf8");
                    for (const key of Object.keys(projectPackage.scripts)) {
                        projectPackageJson.scripts[`${packageNameMap[pkg]}:${key}`] = `yarn workspace @${projectName}/${pkg} ${key}`;
                    }
                }
...
export const isOutputDirectoryEmpty = async (outputFolder) => {
    const files = await readdir(outputFolder);
    if (files.length > 0) {
        throw new Error(`Output directory is not empty. Delete the contents or choose a different project name.`);
    }
};
export const loading = (message) => ora(message).start();
